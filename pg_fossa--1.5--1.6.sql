...
BEGIN
  ...
  ELSEIF a2 IS NULL AND a1 IS NOT NULL THEN
    RETURN a1;
  END IF;
  RETURN ( SELECT COALESCE(ARRAY_AGG(e), '{}'::VARCHAR[]) FROM (SELECT UNNEST(a1) UNION SELECT UNNEST(a2)) AS dt(e) );
END; $$ LANGUAGE PLPGSQL IMMUTABLE;
...
BEGIN
  ...
  ELSEIF a2 IS NULL AND a1 IS NOT NULL THEN
    RETURN a1;
  END IF;
  RETURN ( SELECT COALESCE(ARRAY_AGG(e), '{}'::VARCHAR[]) FROM (SELECT UNNEST(a1) INTERSECT SELECT UNNEST(a2)) AS dt(e) );
END; $$ LANGUAGE PLPGSQL IMMUTABLE;
...
COMMENT ON FUNCTION fossa_edges(VARCHAR(255), VARCHAR[], VARCHAR[], VARCHAR[], VARCHAR[], BOOLEAN, INT) IS 'pg_fossa version 1.6';
COMMENT ON FUNCTION fossa_edges(VARCHAR(255)) IS 'pg_fossa version 1.6';
COMMENT ON FUNCTION fossa_dependencies(VARCHAR(255), VARCHAR[], VARCHAR[], VARCHAR[], VARCHAR[], BOOLEAN, INT) IS 'pg_fossa version 1.6';
COMMENT ON FUNCTION fossa_dependencies(VARCHAR(255)) IS 'pg_fossa version 1.6';
