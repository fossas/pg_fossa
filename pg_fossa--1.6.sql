...
CREATE OR REPLACE FUNCTION update_project_unresolved_issue_count() RETURNS trigger AS $$
DECLARE
  project_locator VARCHAR := (SELECT "projectId" FROM "Revisions" WHERE "locator"=NEW.locator);
  default_branch VARCHAR := (SELECT "default_branch" FROM "Projects" WHERE "locator"=project_locator);
  latest_revision VARCHAR := (SELECT "revision_id" FROM "References" WHERE "project_id"=project_locator AND "type"='branch' AND "name"=default_branch);
BEGIN
  UPDATE "Projects" SET
    "unresolved_issue_count"=revision.unresolved_issue_count,
    "revision_has_error"=revision.error
    FROM (SELECT "unresolved_issue_count", ("error" IS NOT NULL OR "unsupported") AS "error" FROM "Revisions" WHERE "locator"=latest_revision) AS "revision"
    WHERE "locator"=project_locator;
  RETURN NULL;
END; $$ LANGUAGE PLPGSQL VOLATILE;

CREATE OR REPLACE FUNCTION update_all_project_unresolved_issue_counts() RETURNS void AS $$
DECLARE
  project RECORD;
  project_branch VARCHAR;
  latest_revision VARCHAR;
BEGIN
  FOR project IN
    SELECT "locator" FROM "Projects"
  LOOP
    project_branch := (SELECT "default_branch" FROM "Projects" WHERE "locator"=project.locator);
    latest_revision := (SELECT "revision_id" FROM "References" WHERE "project_id"=project.locator AND "type"='branch' AND "name"=project_branch);

    RAISE NOTICE 'Updating %', project.locator;
    UPDATE "Projects" SET
      "unresolved_issue_count"=revision.unresolved_issue_count,
      "revision_has_error"=revision.error
      FROM (SELECT "unresolved_issue_count", ("error" IS NOT NULL OR "unsupported") AS "error" FROM "Revisions" WHERE "locator"=latest_revision) AS "revision"
      WHERE "locator"=project.locator;
  END LOOP;
END; $$ LANGUAGE PLPGSQL VOLATILE;
