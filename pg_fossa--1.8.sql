CREATE FUNCTION fossa_version() RETURNS TEXT as $$
BEGIN
  RETURN '1.8';
...
CREATE FUNCTION array_symmetric_difference(a1 TEXT[], a2 TEXT[]) RETURNS TEXT[] as $$
BEGIN
...
CREATE FUNCTION array_sort_unique(a TEXT[]) RETURNS TEXT[] as $$
BEGIN
  RETURN ARRAY(SELECT DISTINCT UNNEST(a) ORDER BY 1);
END; $$ LANGUAGE PLPGSQL IMMUTABLE;

CREATE FUNCTION array_union(a1 TEXT[], a2 TEXT[]) RETURNS TEXT[] as $$
BEGIN
...
DROP AGGREGATE IF EXISTS array_union_agg(TEXT[]);
...
DROP AGGREGATE IF EXISTS array_intersect_agg(TEXT[]);
CREATE AGGREGATE array_intersect_agg(TEXT[]) (
  SFUNC = array_intersect,
  STYPE = TEXT[]
);

CREATE FUNCTION fossa_child(locator TEXT, unresolved_locator TEXT) RETURNS TEXT as $$
BEGIN
  RETURN CASE WHEN fossa_resolved(locator) THEN locator ELSE unresolved_locator END;
END; $$ LANGUAGE PLPGSQL IMMUTABLE;

CREATE FUNCTION fossa_resolved(locator TEXT) RETURNS BOOLEAN as $$
BEGIN
  RETURN locator != 'NULL';
END; $$ LANGUAGE PLPGSQL IMMUTABLE;

CREATE FUNCTION fossa_edges(locator TEXT, filter_tags TEXT[], filter_excludes TEXT[], filter_origin_paths TEXT[], filter_all_origin_paths TEXT[], filter_unresolved BOOLEAN, maxdepth INT) RETURNS fossa_edge[] AS $$
DECLARE
  current_depth INT := 2;
...
CREATE FUNCTION fossa_edges(locator TEXT) RETURNS fossa_edge[] AS $$
BEGIN
  RETURN fossa_edges(locator, NULL, NULL, NULL, NULL, FALSE, 5);
END; $$ LANGUAGE PLPGSQL;

CREATE FUNCTION fossa_dependencies(locator TEXT, filter_tags TEXT[], filter_excludes TEXT[], filter_origin_paths TEXT[], filter_all_origin_paths TEXT[], filter_unresolved BOOLEAN, maxdepth INT) RETURNS fossa_node[] AS $$
DECLARE
  current_depth INT := 2;
...
CREATE FUNCTION fossa_dependencies(locator TEXT) RETURNS fossa_node[] AS $$
BEGIN
